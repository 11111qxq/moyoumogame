...
com.init = function (stype) {
  ...
  com.nowStype = "stype1"; // 固定为 stype1 样式
  ...
  com.get("box").style.width = com.width + "px"; // 仅使用棋盘宽度
...
com.loadImages = function (stype) {
  ...
  // 确保所有图片加载完成后再更新尺寸
  var imagesToLoad = [com.bgImg, com.dotImg, com.paneImg];
  for (var i in com.args) {
    imagesToLoad.push(com[i].img);
  }

  var loadedCount = 0;
  function checkImagesLoaded() {
    if (loadedCount === imagesToLoad.length) {
      com.show(); // 只调用 com.show 显示页面内容
    }
  }
